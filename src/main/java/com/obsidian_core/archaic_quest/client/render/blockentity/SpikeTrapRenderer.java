...
public class SpikeTrapRenderer implements BlockEntityRenderer<SpikeTrapBlockEntity> {
    ...
    private static final ResourceLocation texture = ArchaicQuest.rl( "textures/block/spike.png" );
    private static final ResourceLocation masterOverlay = ArchaicQuest.rl( "textures/block/spike_trap_overlay.png" );
    private static final ResourceLocation invertMasterOverlay = ArchaicQuest.rl( "textures/block/spike_trap_overlay_inverted.png" );
    ...
    public void render( SpikeTrapBlockEntity spikeTrap, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int overlayTexture ) {
        ...
        if( Minecraft.getInstance().player != null && spikeTrap.getBlockState().getValue( SpikeTrapBlock.SPIKE_MODE ) != SpikeTrapBlock.SpikeMode.NORMAL ) {
            SpikeTrapBlock.SpikeMode spikeMode = spikeTrap.getBlockState().getValue( SpikeTrapBlock.SPIKE_MODE );
            ...
            if( Minecraft.getInstance().player.getItemInHand( InteractionHand.MAIN_HAND ).getItem() == Items.REDSTONE_TORCH ) {
                VertexConsumer vertexConsumer1 =
                        buffer.getBuffer( RenderType.entityCutout( spikeMode == SpikeTrapBlock.SpikeMode.MASTER ? masterOverlay : invertMasterOverlay ) );
