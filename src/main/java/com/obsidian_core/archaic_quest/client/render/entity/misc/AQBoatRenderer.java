...
public class AQBoatRenderer extends BoatRenderer {
    ...
    private final Map<AQBoat.BoatType, Pair<ResourceLocation, ListModel<Boat>>> boatResources;
    
    public AQBoatRenderer( EntityRendererProvider.Context context, boolean hasChest ) {
        super( context, false );
        boatResources = Stream.of( AQBoat.BoatType.values() ).collect( ImmutableMap.toImmutableMap( ( key ) -> key,
                ( boatType ) -> Pair.of( ResourceLocation.fromNamespaceAndPath( ArchaicQuest.MODID, getTextureLocation( boatType, hasChest ) ), createBoatModel( context, boatType, hasChest ) ) ) );
    }
    
    @Override
    public Pair<ResourceLocation, ListModel<Boat>> getModelWithLocation( Boat boat ) {
        if( boat instanceof AQChestBoat )
            return this.boatResources.get( ((AQChestBoat) boat).getBoatType() );
        else
            return this.boatResources.get( ((AQBoat) boat).getBoatType() );
    }
    ...
    private static ModelLayerLocation createLocation( String name, String layer ) {
        return new ModelLayerLocation( ResourceLocation.fromNamespaceAndPath( ArchaicQuest.MODID, name ), layer );
    }
