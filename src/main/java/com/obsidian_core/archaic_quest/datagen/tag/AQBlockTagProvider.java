...
public class AQBlockTagProvider extends BlockTagsProvider {
    ...
    public AQBlockTagProvider( PackOutput packOutput, CompletableFuture<HolderLookup.Provider> lookupProvider, @Nullable ExistingFileHelper existingFileHelper ) {
        super( packOutput, lookupProvider, ArchaicQuest.MODID, existingFileHelper );
    }
    ...
    protected void addTags( HolderLookup.Provider lookupProvider ) {
        ...
        WoodSet.WOOD_SETS.forEach( ( woodSet ) -> {
            ...
            tag( BlockTags.CEILING_HANGING_SIGNS ).add( woodSet.getHangingSign().get() );
            tag( BlockTags.WALL_HANGING_SIGNS ).add( woodSet.getWallHangingSign().get() );
        } );
        
        final IntrinsicHolderTagsProvider.IntrinsicTagAppender<Block> CLIMBABLE = tag( BlockTags.CLIMBABLE );
