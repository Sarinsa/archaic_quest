...
public abstract class AbstractRecipeProvider extends RecipeProvider {
    ...
    protected void stonecutting( ItemLike result, ItemLike ingredient, int count, Consumer<FinishedRecipe> consumer ) {
        ...
        SingleItemRecipeBuilder.stonecutting( Ingredient.of( ingredient ), RecipeCategory.DECORATIONS, result, count )
    ...
    protected void stonecutting( ItemLike result, TagKey<Item> tagIngredient, int count, Consumer<FinishedRecipe> consumer ) {
        ...
        SingleItemRecipeBuilder.stonecutting( Ingredient.of( tagIngredient ), RecipeCategory.DECORATIONS, result, count )
                .unlockedBy( "has_" + tagName, has( tagIngredient ) )
                .save( consumer, resultName + "_from_" + tagName + "_stonecutting" );
    }
    
    protected void smelting( RecipeCategory category, ItemLike result, ItemLike ingredient, float exp, Consumer<FinishedRecipe> consumer ) {
        String ingredientName = itemName( result );
        SimpleCookingRecipeBuilder.smelting( Ingredient.of( ingredient ), category, result, exp, 200 )
                .unlockedBy( "has_" + ingredientName, has( ingredient ) )
                .save( consumer, ArchaicQuest.rl( ingredientName + "_from_smelting" ) );
    }
}
